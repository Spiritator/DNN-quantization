...
for concen_id in concentration_list:
    
    if not os.path.isdir(result_save_folder+'/'+str(concentration_list[concen_id])):
        os.mkdir(result_save_folder+'/'+str(concentration_list[concen_id]))
    ...
    for test_rounds,fr in enumerate(fault_rate_list):
        ...
        param={'model':ref_model,
               ...
               'concentration':concentration_list[concen_id],
...
#                                      fast_gen=True)
        ...
        result_save_file=result_save_folder+'/'+str(concentration_list[concen_id])+'/'+str(fr)+'.csv'
